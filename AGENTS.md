...
└── endpoints/
    ├── mod.rs               # ApiResponse<T> type, error structs
    ├── hello.rs             # GET /
    ...
    └── mensagorgonzola.rs   # GET /mensa-gorgonzola
```

## How to Add a New Endpoint

1. Create `src/endpoints/your_endpoint.rs`
2. Define a response struct deriving `Serialize`, `Deserialize`, `JsonSchema`
3. Implement a Rocket handler with `#[openapi]` macro
4. Register the route in `src/endpoints/mod.rs`
5. Mount it in `src/main.rs`

Use `ApiResponse<YourStruct>` as the return type — it handles both plain text (default) and JSON (`?format=json`) responses automatically.
